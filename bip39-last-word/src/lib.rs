...
/// and passphrase.
///
/// The passphrase is normalized according to BIP39 before seed derivation. The
/// fixed-size entropy input is always valid for a 12-word mnemonic.
...
/// Converts 128 bits of entropy into a BIP39 seed using an empty passphrase.
pub fn seed_from_entropy_with_empty_passphrase(
    language: MnemonicLanguage,
    entropy: &[u8; 16],
) -> [u8; 64] {
    seed_from_entropy(language, entropy, "")
}
...
mod tests {
    ...
    #[test]
    fn derives_the_official_trezor_passphrase_seed() {
        let seed = seed_from_entropy(MnemonicLanguage::English, &[0; 16], "TREZOR");
